...
import { ACTIVE_SUBSCRIPTION_DESCRIPTIONS, PLACEHOLDER_DESCRIPTION, UPCOMING_PAYMENTS_DESCRIPTION } from './subscription-descriptions.js'
...
class SubscriptionInfo {
    ...
    /**
      ...
      * @property {import('./storage/mongodb-storage.js')} storage
      */
    ...
    async getSubscriptionInfo(ids, validBeforeMillis = new Date(2099, 0)) {
        const subscription = await this._getOrReadSubscription(ids)
    ...
    */
    async _getOrReadSubscription(subscriptionOrIds) {
        if (Array.isArray(subscriptionOrIds)) {
            try {
                const sub = await this._storage.get(subscriptionOrIds, true)
                return sub
            } catch (e) {
                if (e.message == 'Not Found') {
                    throw new Error(SubscriptionInfo.ERROR_SUBSCRIPTION_NOT_FOUND, { cause: e })
                } else {
                    throw e
                }
            }
        ...
        return sortedPayments
            .map(payment => {
                ...
                if (payment.alert_name === 'subscription_payment_failed') {
                    ...
                    payment.alert_name === SubscriptionInfo.HYDRATION_SUBSCRIPTION_CREATED) {
                    Object.assign(result, {
                        next_payment: {
